		...
		for key in emotionData.keys():
			if (key != 'neutral'):
				if emotionData[key] > minVal:
					...
					emote = key
		emotionLst.append(emote)
	return emotionLst

def getFaceInfo(json_data):
	faceInfoLst = []
	faceDataLst = json_data['response']['face']
	for faceData in faceDataLst:
...
def drawAnger(background, angerSymbol, leftEyebrow, rightEyebrow, faceInfo, outfile):
	...
	leftOuterX = int(float(faceInfo.eyebrow_info['eyebrowLeftOuter']['x']))
	leftOuterY = int(float(faceInfo.eyebrow_info['eyebrowLeftOuter']['y']))

	rectangleX = int(float(faceInfo.box_info['width']))
	rectangleY = int(float(faceInfo.box_info['height']))
	averageRectLen = (rectangleX + rectangleY) / 4
	angerSymbol = angerSymbol.resize((averageRectLen / 2, averageRectLen / 2), Image.ANTIALIAS)

	
	leftInnerX = int(float(faceInfo.eyebrow_info['eyebrowLeftInner']['x']))
	leftInnerY = int(float(faceInfo.eyebrow_info['eyebrowLeftInner']['y']))
	leftEyebrow = leftEyebrow.resize((averageRectLen, averageRectLen), Image.ANTIALIAS)

	rightInnerX = int(float(faceInfo.eyebrow_info['eyebrowRightInner']['x']))
	rightInnerY = int(float(faceInfo.eyebrow_info['eyebrowRightInner']['y']))
...
def drawHappiness(background, star1, rose, faceInfo, outfile):
	...
	rightOuter = int(float(faceInfo.eye_info['eyeRightOuter']['x']))
	rightInner = int(float(faceInfo.eye_info['eyeRightInner']['x']))
	righty = int(float(faceInfo.eye_info['pupilRight']['y']))
	...
	leftOuter = int(float(faceInfo.eye_info['eyeLeftOuter']['x']))
	leftInner = int(float(faceInfo.eye_info['eyeLeftInner']['x']))
	lefty = int(float(faceInfo.eye_info['pupilLeft']['y']))
	...
	rose_w, rose_h = rose.size
	mouthLeftX = int(float(faceInfo.mouth_info['mouthLeft']['x']))
	mouthLeftY = int(float(faceInfo.mouth_info['mouthLeft']['y']))
	mouthRightX = int(float(faceInfo.mouth_info['mouthRight']['x']))
	mouthRightY = int(float(faceInfo.mouth_info['mouthRight']['y']))
...
def drawSadness(background, leftWater, rightWater, faceInfo, outfile):
	...
	# print(float faceInfo.eye_info['pupilRight']['x']
	righty = int(float(faceInfo.eye_info['pupilRight']['y']))
	rightOuter = int(float(faceInfo.eye_info['eyeRightOuter']['x']))
	rightInner = int(float(faceInfo.eye_info['eyeRightInner']['x']))
...
if __name__ == '__main__':
	...
	for i in range(len(emotionLst)):
		...
		if emotion.lower() == 'happiness':
			...
			image = drawAnger(image, angerSymbol, leftEyebrow, rightEyebrow, faceInfoLst[i], outputImagePath)
		elif emotion.lower() == 'sadness':
			image = drawSadness(image, leftWater, rightWater, faceInfoLst[i], outputImagePath)
		else:
